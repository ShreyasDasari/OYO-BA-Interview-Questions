...
-- 1. Write an SQL query to calculate the total revenue generated by each city. Assume a bookings table with columns: city, room_price, and number_of_nights.
SELECT city, SUM(revenue) as total_revenue from bookings
group by city
ORDER by total_revenue DESC;

-- 2. Write an SQL query to find customers who made more than 3 bookings in the last year. Use a bookings table with columns: customer_id and booking_date.
SELECT customer_id, COUNT(booking_id) as num_of_bookings
FROM bookings
WHERE booking_date >= date('now', '-1 year')
GROUP by customer_id
HAVING COUNT(booking_id) > 3
ORDER by num_of_bookings ASC;

-- 3. Write an SQL query to calculate the cancellation rate for each property. Use a bookings table with columns: property_id, booking_id, and booking_status (confirmed/cancelled).
SELECT property_id, ROUND((SUM(CASE WHEN booking_status = 'cancelled' then 1 else 0 END) * 100 / COUNT(booking_status)), 2) as cancellation_rate
FROM bookings
GROUP BY property_id
ORDER by cancellation_rate DESC;

-- 4. Write an SQL query to find the month with the highest revenue for each city. Assume a bookings table with columns: city, revenue, and booking_date.
WITH monthly AS (
  SELECT
    city,
...
ranked AS (
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY city ORDER BY monthly_revenue DESC, month DESC) AS rn
  FROM monthly
)
...
-- 5. Write an SQL query to find properties with occupancy rates greater than 90%. Assume a properties table with columns: property_id, total_rooms, and occupied_rooms.
SELECT
  property_id,
  AVG(LOWER(booking_status) = 'confirmed') AS occupancy_rate
FROM bookings
GROUP BY property_id
HAVING AVG(LOWER(booking_status) = 'confirmed') > 0.90;
